...
function UsersTab({ token, locale, showToast, onUnauthorizedError }) {
  ...
  }, [onUnauthorizedError, limit, offset, query, token]);
  ...
  const loadUserDetail = useCallback(
    async (nextUserId) => {
      ...
      try {
        ...
        if (onUnauthorizedError(error)) return null;
        setSelectedUser(null);
  ...
  const onSubmitRole = async (event) => {
    ...
    try {
      await updateAdminUserRole({ token, userId: normalizedUserId, role: normalizedNextRole });
  ...
  const onSubmitPremiumGrant = async (event) => {
    ...
    try {
      await grantAdminUserPremium({ token, userId: normalizedUserId, days });
      await refreshCurrentUser();
  ...
  const onRevokePremium = async () => {
    ...
    try {
      await revokeAdminUserPremium({ token, userId: normalizedUserId });
      await refreshCurrentUser();
  ...
  const onSubmitExperience = async (event) => {
    ...
    try {
      await updateAdminUserExperience({ token, userId: normalizedUserId, experience });
      await refreshCurrentUser();
    ...
    <div className={styles.layout}>
      <section className={styles.panel}>
        ...
        <form className={styles.searchForm} onSubmit={onSearchSubmit}>
          ...
          />
          <button type="submit" className={styles.actionButton}>Search</button>
          <button type="button" className={styles.mutedButton} onClick={onClearSearch}>Reset</button>
        </form>
